...
describe("preferences sheet interactions", () => {
  ...
  it("updates footer and copyright preferences", async () => {
    ...
    await userEvent.type(within(dialog).getByRole("textbox", { name: "公司链接" }), "https://example.com")
    ...
    await userEvent.type(within(dialog).getByRole("textbox", { name: "ICP备案号" }), "沪ICP备00000000号")
    await userEvent.type(within(dialog).getByRole("textbox", { name: "ICP备案链接" }), "https://beian.example.com")
    expect(preferences()).toEqual(
      expect.objectContaining({
  ...
  it("updates general preferences and sheet actions", async () => {
    ...
    await userEvent.click(within(dialog).getByRole("button", { name: "Unpin preferences navigation" }))
